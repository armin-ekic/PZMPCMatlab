...
%form
    T1 = [B, A*B, A*A*B]; %Find the controllability matrix to make sure it controllable
    rankT1 = rank(T1); %If this is equal to the dimension of the B matrix we are controllable

    Pe = poly(eig(A));
    T2 = [Pe(1:3); 0,Pe(1:2); 0,0,Pe(1)]; %Find the matrix related to the systems char. polynomial

    T3 = [0,0,1; 0,1,0; 1,0,0]; %Define the flip matrix
...
%Step2: Find the full-state feedback gains in controller form
    ...
    Kz = dP([4,3,2]); %Honestly, I'm not sure what this is doing
